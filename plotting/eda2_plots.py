"""
EDA phase 2 plots — reads tensor_data.npz and emits the full plot suite.

Plain refactor of the legacy npzgen_for_eda_phase2.py: same plots, same
column conventions, just exposed as a callable function so main.py can
invoke it.

Column layout in tensor_data.npz["data"]:
    0  p_b_wind
    1  p_a05_wind
    2  p_a2_wind
    3  p_a3_wind
...
def _h_mask(d: np.ndarray, i: int) -> np.ndarray:
    return (d[:, ZMAX] >= H_EDGES[i]) & (d[:, ZMAX] < H_EDGES[i + 1])


def _t_edges(d: np.ndarray) -> list[float]:
    t = d[:, THICK]
    t25, t50, t75 = np.percentile(t, [25, 50, 75])
    return [float(t.min()), float(t25), float(t50), float(t75), float(t.max() + 0.1)]


def _t_mask(d: np.ndarray, edges: list[float], i: int) -> np.ndarray:
    return (d[:, THICK] >= edges[i]) & (d[:, THICK] < edges[i + 1])


def _plot_hist_grouped(meta, values, title, xlabel, path, use_log, xlim,
                       bin_type="height", te=None):
    if bin_type == "height":
        labels = ["Overall"] + H_LABELS
        masks = [np.ones(len(meta), bool)] + [_h_mask(meta, i) for i in range(3)]
        colors = ["#2196F3", "#2196F3", "#FF9800", "#4CAF50"]
    else:
        edges = te
        labels = ["Overall"] + [f"{edges[i]:.0f}–{edges[i + 1]:.0f}m"
                                for i in range(len(edges) - 1)]
        masks = [np.ones(len(meta), bool)]
        for i in range(len(edges) - 1):
            masks.append(_t_mask(meta, edges, i))
    ...
    for i, (label, mask, color) in enumerate(zip(labels, masks, colors)):
        ...
        if v.size == 0:
            ...
            continue
        ax.hist(v, bins=80, color=color, alpha=0.85, edgecolor="white",
                linewidth=0.3, log=use_log, range=xlim)
    ...
    tag = "log" if use_log else "linear"
    fig.suptitle(f"{title}  [{tag}, x: {xlim[0]} to {xlim[1]}]",
...
def _plot_overlay(datasets, title, xlabel, path, use_log, xlim):
    ...
    ax.set_ylabel("Count (log)" if use_log else "Count")
    tag = "log" if use_log else "linear"
    ax.set_title(f"{title}  [{tag}, x: {xlim[0]} to {xlim[1]}]", fontweight="bold")
    ax.legend()
    fig.tight_layout()
    fig.savefig(path)
    plt.close(fig)
...
def _plot_profile(meta, values, title, ylabel, path, x_col=FZ,
                  x_label="Height z (m)", bin_step=3.0, ylim=None):
    x = meta[:, x_col]
...
def run_eda2_plots(tensor_npz_path: Path, plots_root: Path) -> None:
    d = np.load(tensor_npz_path)["data"]
    if d.size == 0:
        print(f"[eda2-plots] empty tensor_data: {tensor_npz_path}")
        return

    o = _ensure(plots_root)
    te = _t_edges(d)
    print(f"[eda2-plots] {d.shape} rows, thickness edges: {[f'{e:.1f}' for e in te]}")
    ...
    for vtag, use_log, xl in VERSIONS:
        vdir = _ensure(o / "hist_by_height" / vtag)
        for fname, meta, vals, title, xlabel in h_specs:
            _plot_hist_grouped(meta, vals, title, xlabel,
                               vdir / f"{fname}.png", use_log, xl)
    ...
    for vtag, use_log, xl in VERSIONS:
        vdir = _ensure(o / "hist_by_thickness" / vtag)
        for fname, meta, vals, title, xlabel in t_specs:
            _plot_hist_grouped(meta, vals, title, xlabel,
                               vdir / f"{fname}.png", use_log, xl,
                               bin_type="thickness", te=te)

    print("\n=== Overlay ===")
    ov_specs = [
        ("ab_overlay", "(a-b) overlay: 0.5m vs 2m vs 3m", "Δp (shell − surface)", [
            (f"0.5m (mean={ab_05.mean():.3f})", ab_05, COLORS[0]),
            (f"2.0m (mean={ab_2.mean():.3f})", ab_2, COLORS[1]),
            (f"3.0m (mean={ab_3.mean():.3f})", ab_3, COLORS[2]),
        ]),
        ("aa_bb_overlay", "(a,a) vs (b,b): shell W−L vs surface W−L",
         "Δp (windward − leeward)", [
        ...
        ("ab_skew_overlay", "(a,b) skewed overlay", "(shell−surface)/surface", [
            (f"0.5m (mean={np.nanmean(ab_skew_05):.3f})",
             ab_skew_05[np.isfinite(ab_skew_05)], COLORS[0]),
            (f"2.0m (mean={np.nanmean(ab_skew_2):.3f})",
             ab_skew_2[np.isfinite(ab_skew_2)], COLORS[1]),
            (f"3.0m (mean={np.nanmean(ab_skew_3):.3f})",
             ab_skew_3[np.isfinite(ab_skew_3)], COLORS[2]),
        ]),
    ]
    for vtag, use_log, xl in VERSIONS:
        vdir = _ensure(o / "overlay" / vtag)
        for fname, title, xlabel, datasets in ov_specs:
            _plot_overlay(datasets, title, xlabel,
                          vdir / f"{fname}.png", use_log, xl)

    print("\n=== Scatter ===")
    sc_dir = _ensure(o / "scatter")
    for tag, aa_vals, c in [("0.5m", aa_05, COLORS[0]),
                             ("2.0m", aa_2, COLORS[1]),
                             ("3.0m", aa_3, COLORS[2])]:
        fig, ax = plt.subplots(figsize=(7, 7))
        ax.scatter(bb, aa_vals, s=2, alpha=0.15, color=c)
        lim = max(np.percentile(np.abs(bb), 99.5),
                  np.percentile(np.abs(aa_vals), 99.5))
        ...
        ax.legend()
        fig.tight_layout()
        fig.savefig(sc_dir / f"bb_vs_aa_{tag}.png")
        plt.close(fig)

    print("\n=== Profile by z ===")
    pz_dir = _ensure(o / "profile_by_z")
    pz_specs = [
    ...
    for fname, meta, vals, title, ylabel in pz_specs:
        _plot_profile(meta, vals, title, ylabel,
                      pz_dir / f"{fname}.png", ylim=profile_ylim)

    print("\n=== Profile by thickness ===")
    pt_dir = _ensure(o / "profile_by_thick")
    pt_specs = [
        ("aa_0.5m", d, aa_05, "(a,a) shell W−L vs thickness 0.5m", "Mean Δp (W−L)"),
        ("aa_2.0m", d, aa_2, "(a,a) shell W−L vs thickness 2.0m", "Mean Δp (W−L)"),
        ("aa_3.0m", d, aa_3, "(a,a) shell W−L vs thickness 3.0m", "Mean Δp (W−L)"),
        ("bb", d, bb, "(b,b) surface W−L vs thickness", "Mean Δp (W−L)"),
    ]
    for fname, meta, vals, title, ylabel in pt_specs:
        _plot_profile(meta, vals, title, ylabel,
                      pt_dir / f"{fname}.png", x_col=THICK,
                      x_label="Building thickness (m)", bin_step=2.0)

    print(f"\n[eda2-plots] all plots saved under {plots_root}/")


def run_eda2(data_dir: Path, out_dir: Path, workers: int) -> None:
    """Convenience wrapper used by main.py: extract → plots."""
    from .eda2_extract import run_eda2_extract

    tensor_path = run_eda2_extract(data_dir, out_dir, workers)
    if not tensor_path.exists():
        print("[eda2] tensor_data.npz missing, skipping plots")
        return
    run_eda2_plots(tensor_path, out_dir / "plots" / "eda2")
